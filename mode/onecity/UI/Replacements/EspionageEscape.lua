...
function OnButton4()
	...
	if g_type == "Default" then
		...
		tParameters.OnStart = "CustomSpyEscapeTriggered";
		UI.RequestPlayerOperation(Game.GetLocalPlayer(), PlayerOperations.EXECUTE_SCRIPT, tParameters);
		print("UI.RequestPlayerOperation","CustomSpyEscapeTriggered")
		ContextPtr:SetHide(true);
	end
end
...
function OnOpenCustom(plotX,plotY)
	...
	for i, unit in playerUnits:Members() do
		if unit:GetX() == plotX and unit:GetY() == plotY and (unit:GetProperty("ON_A_MISSION") == 1 and (unit:GetProperty("TIME_LEFT") == -1)) then
			spyUnit = unit
			break
		end
	end
...
function Initialize()
	...
	LuaEvents.NotificationPanel_OpenCustomEspionageEscape.Add(OnOpenCustom);
end
Initialize();
