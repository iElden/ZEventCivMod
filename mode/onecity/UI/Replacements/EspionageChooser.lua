...
-- ===========================================================================
-- Support for Additional Mission - Replacing the UnitManager. Objects
-- ===========================================================================
...
function RefreshMissionList()
	...
	if m_city:GetOwner() == Game.GetLocalPlayer() then
		...
		for operation in GameInfo.UnitOperations() do
			if operation.CategoryInUI == "OFFENSIVESPY" or operation.CategoryInUI == "OFFENSIVESPY_EXTRA" then
				local cityPlot:table = Map.GetPlot(m_city:GetX(), m_city:GetY());
				local canStart:boolean, results:table = CustomCanStartOperation(m_spy, operation, cityPlot, false, true)
				if canStart then
...
function AddAvailableOffensiveOperation(operation:table, result:table, pTargetPlot:table)
	...
	print("AddAvailableOffensiveOperation Completed")
end
