...
function CustomCanStartOperation(spy:table, operation:table, targetPlot:table, arg1:boolean, arg2:boolean)
	local canStart:boolean = false
	local results:table = {}
	if operation.CategoryInUI ~= "OFFENSIVESPY_EXTRA" then
		canStart, results = UnitManager.CanStartOperation( spy, operation.Hash, targetPlot, arg1, arg2);
		else
		canStart = true
		results = {}
	end
	return canStart, results
end
...
function RefreshStack()
	print("RefreshStack()",m_operation,m_currentPopupState,m_spy)
	if m_spy and m_currentPopupState ~= EspionagePopupStates.MISSION_COMPLETED then
		-- Update Mission Objective
		-- Need Handler
		Controls.MissionObjectiveLabel:SetText(GetFormattedCustomOperationDetailText(m_operation, m_spy, m_city));
	...
	print("RefreshStack()#2")
	-- Update Mission Duration
	if m_currentPopupState == EspionagePopupStates.MISSION_BRIEFING and m_operation ~= nil then
		print("RefreshStack()#2a",GetCustomTimeToComplete(m_operation, m_spy))
	    Controls.MissionDurationLabel:SetText(Locale.Lookup("LOC_ESPIONAGEPOPUP_TURNS", GetCustomTimeToComplete(m_operation, m_spy)));
		Controls.MissionDurationContainer:SetHide(false);
	elseif m_currentPopupState == EspionagePopupStates.ABORT_MISSION then
		-- Need Handler
		print("RefreshStack()#2b")
	...
	print("EspionagePopup - RefreshStack - Completed")
end
...
function TryRefreshRenewableMission( kMissionHistory:table )
	...
	if m_operation.Hash == UnitOperationTypes.SPY_COUNTERSPY then
		...
		local detailText:string = GetFormattedCustomOperationDetailText(m_operation, m_spy, m_city);
		if detailText ~= "" then
	...
	local canStart:boolean, results:table = CustomCanStartOperation(m_spy, m_operation, pCityPlot, false, true)
	if canStart then
...
function RefreshPossibleOutcomes()
	...
	local resultProbability:table = GetCustomProbabilityTable(m_operation, m_spy, m_pTargetPlot)
...
function RenewMission( spy:table, operationType:number, plotX:number, plotY:number )
	...
	for operation in GameInfo.UnitOperations() do
		if operation.Hash == operationType then
			renewedOps = operation
			break
		end
	end

	if renewedOps.CategoryInUI ~= "OFFENSIVESPY_EXTRA" then
		...
		UnitManager.RequestOperation(spy, operationType, tParameters);
			...
			kParameters.OperationIndex = renewedOps.Index;
			kParameters.OperationType = renewedOps.OperationType;
			kParameters.OperationTurns = renewedOps.Turns;
			kParameters.OperationChance = renewedOps.BaseProbability;
			kParameters.LocalPlayer = Game.GetLocalPlayer();
			kParameters.OnStart = "CustomSpyMissionTriggered";
...
function OnSpyMissionCompleted( playerID:number, missionID:number )
	print("OnSpyMissionCompleted", missionID)
...
-- ===========================================================================
function OnAccept()
	print("OnAccept() - EspionagePopup",m_operation,m_operation.OperationType)
	if m_spy and m_operation then
		if m_operation.CategoryInUI ~= "OFFENSIVESPY_EXTRA" then
			UnitManager.RequestOperation(m_spy, m_operation.Hash,nil)
			else
			local kParameters:table = {};
			kParameters.SpyID = m_spy:GetID();
			kParameters.SpyX =  m_spy:GetX();
...
function Close()
	...
	print("Close() - EspionagePopup - Completed")
end
...
function OnInterfaceModeChanged( oldMode:number, newMode:number )
	print("OnInterfaceModeChanged",oldMode,newMode)
...
function OnMissionExecuted(playerID,UnitID,outcome)
	print("OnMissionExecuted",playerID,UnitID,outcome)
end
